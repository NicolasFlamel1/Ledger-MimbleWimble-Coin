...
int LLVMFuzzerTestOneInput(const uint8_t *data, const size_t size) {
	...
	// Set state to be after start encrypting slate request
	...
	G_io_apdu_buffer[APDU_OFF_INS] = CONTINUE_ENCRYPTING_SLATE_INSTRUCTION;
	...
	BEGIN_TRY {
		...
		TRY {
			...
			processContinueEncryptingSlateRequest(&responseLength, &responseFlags);
		}
