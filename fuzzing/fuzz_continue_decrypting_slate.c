...
int LLVMFuzzerTestOneInput(const uint8_t *data, const size_t size) {
	...
	// Set state to be after start decrypting slate request
	...
	G_io_apdu_buffer[APDU_OFF_INS] = CONTINUE_DECRYPTING_SLATE_INSTRUCTION;
	...
	BEGIN_TRY {
		...
		TRY {
			...
			processContinueDecryptingSlateRequest(&responseLength, &responseFlags);
		}
