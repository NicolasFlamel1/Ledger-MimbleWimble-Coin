...
int LLVMFuzzerTestOneInput(const uint8_t *data, const size_t size) {
	...
	// Set state to be after continue transaction get message signature request
	...
	G_io_apdu_buffer[APDU_OFF_INS] = FINISH_TRANSACTION_INSTRUCTION;
	...
	BEGIN_TRY {
		...
		TRY {
			...
			processFinishTransactionRequest(&responseLength, &responseFlags);
