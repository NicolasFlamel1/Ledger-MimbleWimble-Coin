...
int LLVMFuzzerTestOneInput(const uint8_t *data, const size_t size) {
	...
	G_io_apdu_buffer[APDU_OFF_INS] = CONTINUE_TRANSACTION_GET_PUBLIC_NONCE_INSTRUCTION;
	...
	BEGIN_TRY {
		...
		TRY {
			...
			processContinueTransactionGetPublicNonceRequest(&responseLength, &responseFlags);
		}
