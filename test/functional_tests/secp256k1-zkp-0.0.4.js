...
(function(){function b(d){a.asm=d.exports;N=a.asm.ba;P.unshift(a.asm.g);R--;a.monitorRunDependencies&&a.monitorRunDependencies(R);0==R&&(null!==S&&(clearInterval(S),S=null),T&&(d=T,T=null,d()))}function c(d){b(d.instance)}function e(d){return ka().then(function(){return ea()}).then(function(g){return g}).then(d,function(g){u("failed to asynchronously prepare wasm: "+g);A(g)})}var f={a:z};R++;a.monitorRunDependencies&&a.monitorRunDependencies(R);if(a.instantiateWasm)try{return a.instantiateWasm(f,
b)}catch(d){return u("Module.instantiateWasm callback failed with error: "+d),!1}(function(){return w||"function"!==typeof WebAssembly.instantiateStreaming||U(V)||"function"!==typeof fetch?e(c):fetch(V,{credentials:"same-origin"}).then(function(d){return WebAssembly.instantiateStreaming(d,f).then(c,function(g){u("wasm streaming compile failed: "+g);u("falling back to ArrayBuffer instantiation");return e(c)})})})().catch(p);return{}})();
