...
void getAgePayloadKey(volatile uint8_t *payloadKey, const uint32_t account, const uint32_t index, const uint8_t *ephemeralX25519PublicKey, const uint8_t *encryptedFileKey, const uint8_t *payloadNonce) {
	...
	BEGIN_TRY {
		...
		TRY {
			...
			// Get Ed25519 public key from the Ed25519 private key and throw error if it fails
			...
			// Uncompress ephemeral X25519 public key and throw error if it fails
			...
			// Create a shared private key from the X25519 private key and uncompressed ephemeral X25519 public key and throw error if it fails
